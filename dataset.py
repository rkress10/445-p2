...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        self.image_mean = tuple(np.mean(X, axis=(0,1,2)))
        self.image_std = tuple(np.std(X, axis=(0,1,2)))
    ...
    def transform(self, X):
        ...
        # TODO: Complete this function
        X = np.asarray(X)
        tmp = np.ones(shape=(X.shape[1], X.shape[2], X.shape[3])) # 64, 64, 3
        tmp[0][0] = np.array(list(self.image_mean))
        for i in range(X.shape[1]):
            for j in range(X.shape[2]):
                tmp[i][j] = np.array(list(self.image_mean))
        X = X - tmp
        tmp = np.ones(shape=(X.shape[1], X.shape[2], X.shape[3])) # 64, 64, 3
        tmp[0][0] = np.array(list(self.image_std))
        for i in range(X.shape[1]):
            for j in range(X.shape[2]):
                tmp[i][j] = np.array(list(self.image_std))
        X = X / tmp
